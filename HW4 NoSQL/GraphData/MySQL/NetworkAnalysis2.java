...
public class NetworkAnalysis2 {
	...
	public static int NeighbourCount(int id){
		String sql="select count(*) from facebook where FromNodeId="+id;
		ResultSet rs = null;
		try{			
	...
	public static int ReachabilityCount(int id) throws SQLException{		
		...
		map.put(id,true);
		while (!queue.isEmpty()){
			sql="select ToNodeId from facebook where FromNodeId="+queue.peek();		
			try{				      			      
				   rs=stmt.executeQuery(sql);
				   while (rs.next()) {
					   now=rs.getInt(1);
	...
	/*private static void expandCliques(int len,int k) throws SQLException{
		...
		sql="select ToNodeId from facebook where FromNodeId="+clique.get(0)+" and ToNodeId>FromNodeId";
		rs=stmt.executeQuery(sql);
		while (rs.next()){
			set.add(rs.getInt(1));
		}
		...
		for (int i=1;i<len-1;i++){
			...
			set.clear();
			sql="select ToNodeId from facebook where FromNodeId="+clique.get(i)+" and ToNodeId>FromNodeId";
			rs=stmt.executeQuery(sql);
			while (rs.next()){
				set.add(rs.getInt(1));
			}
		...
		//int []intersect=result.toArray(Integer);
	...
	public static void DiscoverCliques(int k) throws SQLException{
		...
		if (k==1){
			...
			sql="select DISTINCT FromNodeId from facebook order by FromNodeId asc";
			rs=stmt.executeQuery(sql);
			while (rs.next()){
		...
		sql="select * from facebook where FromNodeId<ToNodeId";
		rs=stmt.executeQuery(sql);
	...
	private static int ShortestPath(int startNode) throws SQLException{
		...
		while (!queue.isEmpty()){
			tem=queue.peek();
			sql="select ToNodeId from facebook where FromNodeId="+tem;	
			if (ans<map.get(tem)) {
				ans=map.get(tem);
				endNode=tem;
			}
	...
	public static void main(String[] args) throws IOException, SQLException {
		 ...
		 //  
		   ...
		   //System.out.println(NeighbourCount(0));
		   System.out.println(ReachabilityCount(0));
